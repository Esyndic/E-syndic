...
module.exports = {
  getone: function (callback,id) {
    ...
    add: function (callback, val) {
      const sql = "INSERT INTO `factures` (date,rent,STEG,SONEDE,Topnet,decription,syndic_idsyndic,tenants_id,tenants_syndic_idsyndic) VALUES (?, ?, ?, ?, ?,?,?,?,?)";
      conn.query(
        sql,
        [val.date, val.rent, val.STEG, val.SONEDE, val.Topnet, val.decription, val.syndic_idsyndic, val.tenants_id, val.tenants_syndic_idsyndic],
